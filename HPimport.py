...
def interpolate_nans(df, nLimit=None):
    ...
    print(dfMissing)
    df = df.apply(interpolate, thresh=20)
...
def add_cop_values(df_1min):
    ...
    cop_wm1[df_1min['Eastron01 Total Power'] < 500] = np.nan
    # Set cop_wm2 to NaN if Eastron02 Total Power is below 300 We
    cop_wm2[df_1min['Eastron02 Total Power'] < 500] = np.nan
...
def copy_output_to_automaticreporting():
    src_dir = os.path.join(pExcel, sMeetsetFolder)
    dst_dir = os.path.join(pBase, 'AutomaticReportMaking', 'Input', sMeetsetFolder)
...
if __name__ == "__main__":
    ...
    if bReadPickles: 
        ...
        prefix = '1hour - RV - '
        add_enthalpy_calcualations(df_1hr_newheaders, sMeetsetFolder, prefix=prefix)
        prefix = '1min - RV - '
        add_enthalpy_calcualations(df_1min_newheaders, sMeetsetFolder, prefix=prefix)
        copy_output_to_automaticreporting()
    else:
        ...
        df.to_excel(cmb(pExcel, sMeetsetFolder, filename))
        
        df = interpolate_nans(df, nLimit=20)
        ...
        rowsMask = df[~df['PgasIn'].between(-2.0,70.0)].index
        ...
        if bDebugStopExecutionHere:
            ...
            df_1hr.to_excel(cmb(pExcel, '1hr_' + sMeetsetFolder + datetime.now().strftime('_%Y-%m-%d_%Hh%M.xlsx')))
        ...
        if bWriteExcel:
            ...
            folder_dir = cmb(pExcel, sMeetsetFolder)

            # fpath_1min = save_dataframe_with_dates(df_1min_newheaders, dictHeaderMapping, folder_dir, prefix='1min ')
        
            df_1hr_newheaders = create_output_dataframe(df_1hr, dictHeaderMapping, is_hourly=True)
            # fpath_1hr = save_dataframe_with_dates(df_1hr_newheaders, dictHeaderMapping, folder_dir, prefix='1hour ')
