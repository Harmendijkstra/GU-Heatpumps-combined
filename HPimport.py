...
if len(sys.argv) > 1:
    # Run a specific location
    sMeetsetFolder = sys.argv[1]
    if len(sys.argv) > 2:   
        # Run with specific start date
        sDateStart = sys.argv[2]
        sDateEnd = '2027-12-31'
        bManualReportDates = False
        if len(sys.argv) > 3:
            # Run with specific end date
            sDateEnd = sys.argv[3]
    else:
        bRunPreviousWeek = True
else:
...
#TODO Lennart check for Nunspeet and Wijhe if minWPpower and maxpower are also OK. Do these coefficients all go for Deventer only?
minimum_WP_power = 0 #500 #Deprecated - Minimum power in We to consider the WP to be running
...
#TODO Lennart: I think this only goes for Deventer, possibly other values for other locations
...
#TODO Lennart: for Wijhe/Nunspeet, correct Belimo values before 13:00 to 13:15 on 3-Dec-2025
        #Nunspeet: flow x1000 and totalizer x1000 (both in m3 instead of liters)
        #Wijhe: same correction for totalizer. Flow rates: only Belimo03 x1000, 01-02 are correct already.
        #Timestamp cutoff: Nunspeet only correct before 2025-12-03 13:11:00, Wijhe only correct before 2025-12-03 13:17:00
        if location == 'Nunspeet':
            print('Correcting Nunspeet Belimo values before 2025-12-03 13:11:00')
            pattern = r"^Belimo0. Flow.+"
            correction_cols = [col for col in df.columns if re.match(pattern, col)]
            df.loc[df.index < pd.Timestamp('2025-12-03 13:11:00'), correction_cols] *= 1000
        elif location == 'Wijhe':
            print('Correcting Wijhe Belimo values before 2025-12-03 13:17:00')
            pattern = r"^Belimo0. FlowTotalL"
            correction_cols = [col for col in df.columns if re.match(pattern, col)] + ['Belimo03 FlowRate']
            df.loc[df.index < pd.Timestamp('2025-12-03 13:17:00'), correction_cols] *= 1000
        ...
        filename = 'Before_corrections ' + sMeetsetFolder + datetime.now().strftime('_%Y-%m-%d_%Hh%M') + '.xlsx'
